...
function uploadFile(fileUri) {
  ...
  axios({
          method: 'post',
          url: 'http://192.168.31.154:8000/images/upload?X-TRENA-KEY=0a944fb8-2bbc-4f03-a81a-bf84899cd4f2',
          data: formData,
          headers: {
...
const addCollect = async (data, onUploadProgress) => {
  ...
  data.images.forEach((image, index) => {
    photoData.append("teste" + index, blobCreationFromURL(image));
  });
  ...
  return await client.post(endpointPhotoUpload, photoData);
  // console.log(res);
  // for (const imageUri of data.images) {
  //   // uploadImage(imageUri);
  //   //Post Photo
